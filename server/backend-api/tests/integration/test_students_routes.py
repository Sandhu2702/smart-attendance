...
async def test_upload_student_image(client: AsyncClient, auth_token):
    ...
    response = await client.post("/students/me/face-image", files=files, headers=headers)
    
    if response.status_code == 404:
         # Fallback for testing routing
         response = await client.post("/students/upload-face", files=files, headers=headers)
    ...
    assert response.status_code not in (404, 405), f"Upload failed with {response.status_code}: {response.text}"
